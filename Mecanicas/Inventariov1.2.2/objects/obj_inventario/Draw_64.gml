...
for (var i = 0; i<total_slots; i++){ 
	...
	if grid_itens[# Infos.Item, i] != -1{
		var item_index = grid_itens[# Infos.Item, i];
		draw_sprite_ext(spr_itens,grid_itens[# 0, i], _slotsx, _slotsy, escala, escala, 0, c_white, 1)
		
		draw_set_font(Font1)
		draw_set_halign(fa_center)
		var quantidade = grid_itens[# Infos.Quantidade, i];
        draw_text(_slotsx + 20, _slotsy + 10, string(quantidade));
	}
	ix++ 
	if(ix >= slots_h){
		ix =0 iy++ 
